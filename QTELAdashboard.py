...
st.title("Analisis Fasilitas Twin Tower")
...
with tab1:
        ...
        st.markdown(
            ...
            <div style="text-align:left; margin-bottom:10px;">
                <h1 style="color:#0d6efd; font-size:40px; margin:0; line-height:1.05;">
                    Ternyata, tingkat kepuasan mahasiswa terhadap fasilitas Twin Tower mencapai sekitar 70% yang menyebabkan peningkatan motivasi mahasiswa dalam belajar
                </h1>
    ...
    with col_right:
        ...
        if "Peningkatan_Motivasi" in df_filtered.columns:
            ...
            fig_motiv = px.bar(df_motiv, x="Skor", y="Jumlah", text="Jumlah", color_discrete_sequence=["#FF69B4"], 
    ...
    if "Fakultas" in df_filtered.columns and "Kepuasan_Keseluruhan" in df_filtered.columns:
        ...
        fig_fak = px.bar(avg_per_fak, x="Fakultas", y="Kepuasan_Keseluruhan", text=avg_per_fak["Kepuasan_Keseluruhan"].round(2), title="", color_discrete_sequence=["#FF69B4"])
...
with tab2:
    ...
    st.subheader("Perbandingan Semua Faktor")
    if available_factors:
        radar_data = df_filtered[available_factors].mean().reset_index()
        radar_data.columns = ["Faktor", "Skor"]
        fig_radar = go.Figure(data=go.Scatterpolar(r=radar_data["Skor"], theta=radar_data["Faktor"], fill='toself'))
        fig_radar.update_layout(polar={'radialaxis': {'range': [0, 5]}}, template="plotly_white")
        st.plotly_chart(fig_radar, use_container_width=True)
        st.markdown("**Penjelasan**: Bentuk ini menunjukkan kekuatan setiap faktor. Semakin besar area, semakin baik faktor itu.")
...
with tab3:
    ...
    if "Kepuasan_Keseluruhan" in df_filtered.columns:
        ...
        fig_hist = px.histogram(df_filtered, x="Kepuasan_Keseluruhan", nbins=10, title="",  color_discrete_sequence=["#FF6600"])
